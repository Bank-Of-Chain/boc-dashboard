...
const dataMerge = (account, chain, vault, tokenType, requests) => {
  ...
  return Promise.all([
    ...
    .then((rs) => {
      const [day7Apy, day30Apy, tvls, monthProfits, profit, balanceOfToken] =
        rs;
      ...
      for (let i = 0; i < 12; i++) {
        const month = moment()
          ...
          .format("YYYY-MM");
        const profit =
          find(monthProfits, (item) => item.month === month)?.profit || 0;
        monthProfitsData.push(
          toFixed(profit, USDI_BN_DECIMALS, displayDecimals)
        );
      }
      const nextData = {
        day7Apy,
...
export default function usePersonalData(tokenType) {
  ...
  useEffect(() => {
    ...
    const tokenContract = new ethers.Contract(
      initialState.tokenAddress,
      ABI,
      jsonRpcProvider
    );
    requests.push(
      tokenContract
        .balanceOf(initialState?.address)
        .catch(() => BigNumber.from(0))
    );
    dataMerge(
      initialState?.address?.toLowerCase(),
      initialState?.chain,
      initialState?.vault,
      tokenType,
      requests
