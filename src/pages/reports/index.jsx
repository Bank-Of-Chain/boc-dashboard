...
const Reports = () => {
  ...
  const {
    ...
    loading: roleLoading,
    error: roleError,
  } = useAdminRole(initialState.address);
  ...
  const reportCancel = async (id) => {
    ...
    const headers = await getSignatureHeader(
  ...
  useEffect(() => {
    ...
    if (
      !isEmpty(chain) &&
      !isEmpty(walletChainId) &&
      !isEqual(chain, walletChainId)
    ) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        // 如果报告超过1天，则不能进行驳回
        if (
          moment(geneTime).isBetween(moment().subtract(1, "days"), moment())
        ) {
          if (isAdmin && type === 0) {
            if (roleLoading) {
            ...
            <Tooltip
              title={
                <div>
                  ...
                  <span>
                    RejectTime:{" "}
                    {moment(rejectTime).format("yyyy-MM-DD HH:mm:ss")}
                  </span>
                </div>
              }
            >
  ...
  const aprBefore =
    totalAssets === undefined
      ? 0
      : (
          (365 * 100 * sum(originalGain)) /
          (totalAssets * durationDays)
        ).toFixed(2);
  const aprAfter =
  ...
  return (
    <GridContent>
        ...
        <Row>
          <Col span={24}>
            ...
            >
              ...
              >
                {isExec === 0 &&
                  `Not execute${forcedExecuted ? " (enforced)" : ""}`}
                {isExec === 1 && "Execute"}
              </Descriptions.Item>
            ...
            <Descriptions>
              <Descriptions.Item label="APR">
                <span className={styleMap[aprVariation > 0]}>
                  {aprVariation}%{" "}
                  {aprVariation !== 0 && iconRender(aprVariation > 0)}
                </span>
              </Descriptions.Item>
              ...
              <Descriptions.Item label="Profits Before">
                <span className={styleMap[false]}>
                  {sumOriginalGain.toFixed(6)}
                </span>
              </Descriptions.Item>
              ...
              <Descriptions.Item label="Harvest Fee">
                <span className={styleMap[sumHarvestFeeVariation > 0]}>
                  ...
                  {sumHarvestFeeVariation !== 0 &&
                    iconRender(sumHarvestFeeVariation > 0)}
                </span>
              </Descriptions.Item>
              <Descriptions.Item label="Harvest Fee Before">
                <span className={styleMap[false]}>
                  {sumOriginalHarvestFee.toFixed(6)}
                </span>
              </Descriptions.Item>
              <Descriptions.Item label="Harvest Fee After">
                <span className={styleMap[true]}>
                  {sumHarvestFee.toFixed(6)}
                </span>
              </Descriptions.Item>
            </Descriptions>
            <Divider orientation="left" plain>
      ...
      <Modal
        ...
        onOk={() =>
          changeNetwork(initialState.chain, userProvider, getWalletName())
        }
        onCancel={hideModal}
        okText="ok"
        cancelText="close"
      >
        <p>
          ...
          <span style={{ color: "red", fontWeight: "bold" }}>
            {CHIANS_NAME[initialState.walletChainId] ||
              initialState.walletChainId}
          </span>
        </p>
        <p>
          Current Chain:{" "}
