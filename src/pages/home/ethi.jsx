...
const ETHiHome = () => {
    ...
    Promise.all([
      ...
      .then(([data, estimateApys]) => {
        ...
        option.xAxis.data = option.xAxis.data.map((item) => {
          ...
          const value = `${moment(item)
            .add(1, "days")
            .format("YYYY-MM-DD HH:mm")} (UTC)`;
          xAxisLabels[value] = moment(item)
            .add(1, "days")
  ...
  const price = () => {
    if (
      isEmpty(pegToken) ||
      pegToken?.totalSupply === "0" ||
      isEmpty(vault?.totalAssets)
    )
      return "1";
    if (!isEmpty(vaultBuffer)) {
      if (vault.isAdjust) {
        return toFixed(
          BigNumber.from(vault.totalAssets).sub(vaultBuffer.totalSupply),
          pegToken?.totalSupply,
          6
        );
      }
    }
  ...
  if (vaultData) {
    ...
    vaultData.totalValueInVault = BN(vaultData.totalAssets)
      .minus(strategyTotal)
      .toString();
    vaultData.strategies.map(
      (item) => (item.totalValue = item.debtRecordInVault)
    );
  }
